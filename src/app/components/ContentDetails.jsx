...
import { ArrowLeft, Download, FileIcon, ImageIcon, VideoIcon } from 'lucide-react'
...
export function ContentDetails({ content }) {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'description':
        return <p className="text-slate-300">{content.description || 'No description available'}</p>
      case 'related':
        return (
        ...
        return (
          <ul className="space-y-2">
            {content.files && content.files.length > 0 ? (
              content.files.map((file, index) => (
                <li key={index} className="flex items-center justify-between bg-slate-800 p-3 rounded-lg">
                  <span className="text-white flex items-center gap-2">
                    ...
                    {file.type === 'document' && <FileIcon className="w-5 h-5" />}
                    {file.name}
                  </span>
              ...
              <p className="text-slate-300">No files available</p>
            )}
          </ul>
        )
      default:
